...
public interface RFC4Repository extends JpaRepository<Consumos, Integer> {
        ...
        @Query(value = "select * from (SELECT consumos.tiposservicio_tipo, consumos.costo, reservas.fecha_salida AS fecha  "
                        +
                        "FROM RESERVAS " +
                        "INNER JOIN RESERVAN " +
                        "ON reservas.id = reservan.habitaciones_id " +
                        "INNER JOIN CONSUMOS " +
                        "ON consumos.habitaciones_id = reservan.habitaciones_id " +
                        "WHERE consumos.tiposservicio_tipo = :tipo) WHERE ROWNUM <= 1000 ", nativeQuery = true)
