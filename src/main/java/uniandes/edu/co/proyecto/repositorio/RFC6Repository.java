...
public interface RFC6Repository extends JpaRepository<Consumos, Integer> {
        ...
        @Query(value = "WITH CountMax AS ( " + //
                        ...
                        "FROM CountMax " + //
                        "WHERE conteo = (SELECT MAX(conteo) FROM CountMax)) WHERE ROWNUM <= 100 ", nativeQuery = true)
        ...
        @Query(value = "WITH CountMax AS ( " + //
                        ...
                        "select * from (SELECT fecha " + //
                        "FROM CountMax " + //
                        "WHERE costo_total = (SELECT MAX(costo_total) FROM CountMax)) WHERE ROWNUM <= 100 ", nativeQuery = true)
        Object[] darIngreso();
        ...
        @Query(value = "WITH CountMin AS ( " + //
                        ...
                        "select * from (SELECT fecha " + //
                        "FROM CountMin " + //
                        "WHERE conteo = (SELECT MIN(conteo) FROM CountMin)) WHERE ROWNUM <= 100 ", nativeQuery = true)
