...
public interface RFC1Repository extends JpaRepository<Consumos, Integer> {
    ...
    @Query(value = " SELECT * FROM (SELECT consumos.habitaciones_id, sum(consumos.costo) FROM RESERVAS " +
            "INNER JOIN RESERVAN " +
            ...
            "GROUP BY consumos.habitaciones_id ORDER BY consumos.habitaciones_id) WHERE ROWNUM <= 1000", nativeQuery = true)
